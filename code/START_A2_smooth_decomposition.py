...
def START_A2_smooth_decomposition(dataset_name):
    ...
    output_subject_wise = dict()
    PKL_output_subject_wise = os.path.join(resultsPath, 'output_subject_wise.pkl')

    redo_smoothing = False
    ...
    for smoothing_kernel in smoothing_kernels:
        ...
        output_subject_wise[f'{smoothing_kernel}_mm'] = {}
        ...
        for roi in ROIs:
            ...
            variances = decompose_subj_group(data_region, cond_vec, part_vec, separate='subject_wise')
            output_subject_wise[f'{smoothing_kernel}_mm'][roi] = variances

    with open(PKL_output, 'wb') as pk:
        pickle.dump(output, pk)

    with open(PKL_output_subject_wise, 'wb') as pk:
        pickle.dump(output_subject_wise, pk)
...
if __name__=='__main__':
    ...
    if len(sys.argv) > 1:
        ...
        START_A2_smooth_decomposition('MDTB')
