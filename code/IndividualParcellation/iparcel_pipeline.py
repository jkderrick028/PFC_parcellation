...
if __name__ == "__main__":
    ...
    dtrain, dtest, cond_v, part_v, sub_i, info_df = split_dataset(data, info, [0], [1])

    # HBP parcellations - get fitted V
    ar_model = ar.build_arrangement_model(U, prior_type='logpi', atlas=atlas, sym_type='asym')
    parcels_hbp_full, parcels_hbp_data, V, X = get_indiv_parcellation(ar_model, atlas, dtrain, cond_v, part_v, sub_i)
    hbp_df = info_df.copy()
    hbp_df['method'] = 'HBP'

    # DR parcellations
    U_ica = dr.group_ica(dtrain[0], n_components=17) # group ICA
    parcels_dr_gica, dr_df = get_indiv_parcels('DR', U_ica, dtrain, info_df)
    parcels_dr_hbp, dr_df = get_indiv_parcels('DR', U.T, dtrain, info_df)

    # SDL parcellations
    y = pt.matmul(pt.linalg.pinv(X), pt.tensor(dtrain, dtype=pt.get_default_dtype()))
    parcels_sdl_online, sdl_df = get_indiv_parcels('SDL', U.T, y, info_df, sdl_method='online', sdl_init='hbp', V=V, X=X)
    parcels_sdl_lasso, sdl_df = get_indiv_parcels('SDL', U.T, y, info_df, sdl_method='mt_lasso', sdl_init='hbp', V=V, X=X)
    parcels_sdl_enet, sdl_df = get_indiv_parcels('SDL', U.T, y, info_df, sdl_method='mt_enet', sdl_init='hbp', V=V, X=X)
    ...
    eval_dr_uica, dr_df = evaluate_parcels(U, parcels_dr_gica, dtest[0], atlas, dr_df, 'DCBC_GICA_U')
    eval_dr_uhbp, dr_df = evaluate_parcels(U, parcels_dr_hbp, dtest[0], atlas, dr_df, 'DCBC_HBP_U')
    eval_sdl_online, sdl_df = evaluate_parcels(U, parcels_sdl_online, dtest[0], atlas, sdl_df, 'DCBC_online')
    eval_sdl_lasso, sdl_df = evaluate_parcels(U, parcels_sdl_lasso, dtest[0], atlas, sdl_df, 'DCBC_MT_lasso')
    eval_sdl_enet, sdl_df = evaluate_parcels(U, parcels_sdl_enet, dtest[0], atlas, sdl_df, 'DCBC_MT_enet')
    ...
    dcbc_df = pd.DataFrame({'Group': hbp_df.DCBC_group,
                    ...
                    'DR (GICA U)': dr_df.DCBC_GICA_U,
                    'DR (HBP U)': dr_df.DCBC_HBP_U,
                    'SDL (online)': sdl_df.DCBC_online,
                    'SDL (MT lasso)': sdl_df.DCBC_MT_lasso,
                    'SDL (MT enet)': sdl_df.DCBC_MT_enet})
