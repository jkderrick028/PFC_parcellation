...
X_individuals = X_individuals[:, [0, 2, 29, 31], :]
task_conds = [task_conds[x] for x in [0, 2, 29, 31]]
...
for smoothing_kernel in smoothing_kernels:
    ...
    for subjI in np.arange(n_subjects):
        for condI in np.arange(n_conds):
            ...
            cifti_subj_cond_smooth_residuals = atlas.data_to_cifti(X_individuals[subjI, condI, :].reshape(1, -1))
            cifti_file_name = os.path.join(resultsPath, 'cifti_sub-%02d_cond_%02d_%dmm_residuals.dscalar.nii' % (subjI, condI, smoothing_kernel))
            nib.save(cifti_subj_cond_smooth_residuals, cifti_file_name)

    # os.remove(os.path.join(resultsPath, '*.dscalar.nii'))
