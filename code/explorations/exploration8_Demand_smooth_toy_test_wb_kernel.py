...
data_unsmoothed = 3 * np.ones(data_unsmoothed.shape)
...
for smoothing_kernel in smoothing_kernels:
    for subjI in np.arange(n_subjects):
        for condI in np.arange(n_conds):
            ...
            wb_cmd = f'wb_command -cifti-smoothing {cifti_file_name} {smoothing_kernel} {smoothing_kernel} COLUMN {cifti_smoothed_file_name} -fwhm -left-surface {flat_surf_L} -right-surface {flat_surf_R}'
