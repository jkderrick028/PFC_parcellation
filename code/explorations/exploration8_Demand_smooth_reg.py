...
"""
Smooth then subtract out the entire cond x vertices pattern
"""
...
for smoothing_kernel in smoothing_kernels:
    for subjI in np.arange(n_subjects):
        ...
        for condI in np.arange(n_conds):
            ...
            X.append(atlas.cifti_to_data(cifti_subj_cond_smoothed))

        X = np.array(X)
        X[np.where(np.isnan(X))] = 0
        X = X.reshape(-1, 1)
        y = X_individuals[subjI].reshape(-1, 1)

        reg = LinearRegression(fit_intercept=False).fit(X,y)
        # reg = LinearRegression(fit_intercept=True).fit(X,y)

        regressed = (reg.coef_ * X + reg.intercept_).flatten().reshape(n_conds, n_vertices)
        X_individuals_regressed[subjI] = regressed        
        X_individuals[subjI] = X_individuals[subjI] - X_individuals_regressed[subjI]
